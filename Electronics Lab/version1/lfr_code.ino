...
int calculateError() {
  ...
  // 1. All White Condition: Continue PID using the last known error direction
  if (!s1 && !s2 && !s3 && !s4 && !s5) {
    return previousError; 
  }

  // 2. Grid Intersections (Crossroads)
  if (s1 && s2 && s3 && s4 && s5) return 0; // Full crossroad
  if (s2 && s3 && s4) return 0;             // Thick line / crossing

  // 3. Grid Corners (90-Degree Junctions)
  if (s1 && s2 && s3 && !s4 && !s5) return -45; // Sharp Left L-Junction or T-Junction
  if (!s1 && !s2 && s3 && s4 && s5) return 45;  // Sharp Right L-Junction or T-Junction

  // 4. Fine-Tuned Proportional Line Following
  // Left Deviations
